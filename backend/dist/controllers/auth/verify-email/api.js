...
exports.resendVerificationEmail = exports.confirmEmailVerification = exports.sendVerificationEmail = void 0;
...
const sendVerificationEmail = async (req, res) => {
    try {
        ...
        await redis_1.default.setEx(`verify-email:${token}`, 900, JSON.stringify({ email, role, name, password }));
...
const confirmEmailVerification = async (req, res) => {
    try {
        ...
        const cached = await redis_1.default.get(`verify-email:${token}`);
        if (!cached) {
        ...
        if (role === "ADMIN") {
            ...
            await redis_1.default.del(`verify-email:${token}`);
            await (0, sendEmail_1.sendEmail)({
                to: email,
        ...
        if (role === "USER") {
            ...
            const passwordHash = await (0, crypto_2.hashPassword)(tempPassword);
            ...
            await redis_1.default.del(`verify:${token}`);
...
const resendVerificationEmail = async (req, res) => {
    try {
        ...
        const existing = await redis_1.default.keys(`verify:*`);
        ...
        if (tokenKey) {
            const ttl = await redis_1.default.ttl(tokenKey);
            if (ttl && ttl > 0) {
                return res.status(429).json({ message: `Please wait ${ttl} seconds before requesting a new verification email` });
            }
        }
        const token = crypto_1.default.randomBytes(32).toString("hex");
        await redis_1.default.setEx(`verify:${token}`, 900, JSON.stringify({ email, role }));
        const link = `${process.env.FRONTEND_URL}/verify?token=${token}`;
        await (0, sendEmail_1.sendEmail)({
            to: email,
